...
%% ============================================================
%% Pass demo option to graphicx before Beamer loads it:
%% this silently substitutes grey boxes for any missing image files,
%% so qcfigures/* can be absent without compile errors.
...
%% ============================================================
%% Inserted from qmlnordita.tex (after slide 4)
%% ============================================================
\begin{frame}[plain,fragile]
\frametitle{Thanks to many}

\begin{alertblock}{Many good friends and colleagues}
Jane Kim (ANL), Patrick Cook (MSU), Danny Jammooa (MSU), Dean Lee (MSU), Ryan LaRose (MSU), Nick Cariello (MSU), Bryce Fore (ANL), Alessandro Lovato (ANL), Stefano Gandolfi (LANL), Francesco Pederiva (UniTN), Arnau Rious (Barcelona), Giuseppe Carleo (EPFL), 
...
\begin{frame}[plain,fragile]
\frametitle{A long digression and background (another talk): Di Vincenzo criteria}
...
\begin{frame}{Two-Electron 2D Schrödinger via DVR and FCI with Slater Determinants}
...
%% ============================================================
%% Inserted from limits.tex (after slide 29)
%% ============================================================

%% ── Limits/variational slides ──────────────────────────────────────────────
...
\begin{frame}{The Variational Principle}
  For a normalised trial wave function $\psi_\theta$:
  \[
    E[\psi_\theta]
...
\begin{frame}{Two PINN Training Strategies}
  ...
  \[
    \psi_\theta(\mathbf{x}) \approx \text{wave function}
  \]
    ...
    \column{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item Residual small but solution error large
              (stability issue — see earlier slides)
      \end{itemize}
    \column{0.44\textwidth}
  ...
  \begin{columns}[T]
    \column{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Unnormalised networks can give arbitrarily low apparent energies
      \end{itemize}
      \vspace{0.3cm}
    ...
    \column{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Soft BCs can be dominated by other loss terms
      \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Variational vs.\ Residual: Summary Comparison}
  ...
  \begin{tabular}{p{3.2cm} p{4.0cm} p{4.0cm}}
    ...
    Normalisation      & Automatic via quotient    & Must be added \\
...
%% ─── Limits slide 8 ────────────────────────────────────────────────────────
\begin{frame}{Implications for the Slater--Jastrow--Neural Ansatz}
  \begin{columns}[T]
    \column{0.52\textwidth}
      The PINN+BF / PINN+CTNN pipeline uses
      \textbf{residual-based pretraining followed by SR tail}
      (see slide 29):
      \begin{itemize}
        \item Pretraining phase: PDE residual loss —
              \alert{no variational guarantee}
        \item SR tail: energy minimisation —
              \alert{variational principle restored}
      \end{itemize}
      \vspace{0.2cm}
      The SR tail is therefore not merely an optimisation improvement;
      it is essential for restoring the variational upper-bound property.
    \column{0.44\textwidth}
      \begin{block}{Why this matters}
        Energies reported in Table~2 are obtained \emph{after} the SR
        tail, so they carry variational meaning and can be compared to
        DMC on a firm theoretical basis.
      \end{block}
      \vspace{0.2cm}
      \begin{alertblock}{Design principle}
        Always end training with an energy-minimisation phase if
        variational bounds are required.
      \end{alertblock}
  \end{columns}
\end{frame}
    ...
    \column{0.52\textwidth}
      \begin{block}{When variational principle is preserved}
        \begin{itemize}
          ...
          \item Normalisation enforced via quotient
          \item Antisymmetry exact (Slater determinant)
          \item SR / natural-gradient optimisation tail
        \end{itemize}
      \end{block}
    \column{0.44\textwidth}
      \begin{alertblock}{When it is not}
        \begin{itemize}
          ...
          \item Soft normalisation penalty
...
\begin{frame}{Exact Diagonalisation Reference}
  \begin{columns}[T]
    \column{0.52\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Lowest $n_{\rm ci}$ retained; full $N$-body Hamiltonian
              (kinetic $+$ Coulomb) built and diagonalised
      \end{enumerate}
      \vspace{0.3cm}
      \begin{block}{Convergence criterion}
        SP energies $\approx \omega$ (DVR resolves the HO ground state)
      \end{block}
    \column{0.44\textwidth}
      ...
      \begin{tabular}{c|ccc|c}
        ...
        8    & --  & --  & $16\!\times\!16$ & 0.842$^\dagger$ \\
        \bottomrule
      \end{tabular}
      \normalsize
      \vspace{0.25cm}
      \begin{alertblock}{$N=8$ caveat}
        $n_{\rm sp}^8$ too large for efficient enumeration at large $n_x$.
        $E_0^{(N=8)}=10.512$\,au is a \emph{provisional} lower estimate of
        the CI upper bound ($\dagger$ under-resolved).
      \end{alertblock}
  \end{columns}
\end{frame}
...
\begin{frame}{Network Architecture}
  \begin{columns}[T]
    \column{0.48\textwidth}
      ...
      \end{block}
    \column{0.48\textwidth}
      \begin{block}{SpectralG network (imaginary-time evolution)}
        \begin{itemize}
          \item 3 temporal modes $\{A_k(\tau)\}$ + constant
          \item Spatial network: 2 layers, width 32
          \item FiLM conditioning on $\tau$ through spectral mode
                amplitudes
      ...
      \begin{alertblock}{Wavefunction form}
        ...
        Singlet: $\log|\psi_{\rm singlet}| = \mathrm{logaddexp}(J(\mathbf{r}_1,\mathbf{r}_2),\,J(\mathbf{r}_2,\mathbf{r}_1))$
      \end{alertblock}
  \end{columns}
\end{frame}
...
\begin{frame}{Training Protocol}
  \begin{columns}[T]
    \column{0.50\textwidth}
      ...
      \begin{itemize}
        \item Optimiser: Adam, $\text{lr}=10^{-4}$, cosine warm-up
              (200--400 epochs), $\text{lr}_{\rm min}=0.1\times\text{lr}$
        \item Batch size $n_{\rm coll}$: 512 ($N\leq4$), 1024 ($N=6$),
              2048 ($N=8$)
        \item Loss: energy-variance with $\alpha$ annealing $0\to1$
              over first 60\% of epochs
        \item Laplacian: finite difference ($h=0.01$) for $N>4$
        \item Local energy clip: $5.0\times\mathrm{MAD}(E_L)$
        \item Gradient clip: $0.2$
      \end{itemize}
    \column{0.46\textwidth}
      \begin{block}{Annealed loss}
        \[
          \mathcal{L} = \alpha\,(\langle E_L\rangle - E_{\rm ref})^2
          + (1-\alpha)\,\mathrm{Var}(E_L)
        \]
        $\alpha=0$: variance min.\ (converge to eigenstate)\\
        $\alpha\to1$: anchor to CI reference $E_{\rm ref}$
      \end{block}
      \vspace{0.2cm}
      \begin{block}{Imaginary-time profiles}
        \renewcommand{\arraystretch}{1.2}
        \scriptsize
        \begin{tabular}{l|cc}
          \toprule
          ...
          \midrule
          GS epochs   & 250  & 600   \\
          PDE epochs  & 3000 & 10000 \\
          $n_{\rm precompute}$ & 3072 & 8192 \\
          \bottomrule
        \end{tabular}
        \normalsize
      \end{block}
...
%% ─── Slide 5 ────────────────────────────────────────────────────────────────
\begin{frame}{Ground-State Energy Accuracy: Scaling $N=2$--$6$}
  \begin{center}
  ...
  \begin{alertblock}{Key result}
    ...
    with three independent seeds. Seed spread decreases with $N$ in
    relative units.
  \end{alertblock}
\end{frame}
