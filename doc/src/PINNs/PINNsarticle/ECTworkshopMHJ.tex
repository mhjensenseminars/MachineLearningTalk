...
\frametitle{Di~Vincenzo Criteria for Quantum Computing}
...
\begin{frame}{What is Reinforcement Learning? (Used here to maximise entanglement)}
\begin{block}{Setting: electrons on helium}
In our double-well setup, the \textbf{agent} controls the electrode voltages $\{V_i\}$; the \textbf{reward} is the total entanglement entropy of the two-electron state. RL finds the voltage configuration that maximises quantum correlations.
\end{block}
...
objective is to maximise the sum of expected rewards and thereby
...
\begin{frame}{Activation Functions and Higher-Order Derivatives}
  \begin{columns}[T]
    ...
    \column{0.46\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/activation_derivatives}
  \end{columns}
\end{frame}
...
\begin{frame}{Energy-Partition Diagnostics (Figures)}
  \begin{columns}[T]
    \column{0.49\textwidth}
      ...
      \vspace{2pt}
      {\scriptsize (a) $\Gamma(\omega)=\langle V_{\rm int}\rangle/\langle T\rangle$ (log--log). Interaction dominance rises sharply as $\omega\to 0$.}
    \column{0.49\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/energy/ratios_allN_classical.png}
      \vspace{2pt}
      {\scriptsize (b) $\mathcal{C}(\omega)=2\langle V_{\rm trap}\rangle/\langle V_{\rm int}\rangle$. Larger $N$ approaches the quasi-classical limit more rapidly.}
  \end{columns}
...
\begin{frame}{PINNs vs.\ VMC: Complementary Strengths}
  \begin{block}{Why PINNs generalise for physics}
    ...
    symmetry, conservation laws, and the PDE structure are built directly
    ...
    \column{0.48\textwidth}
      \begin{block}{VMC advantages}
        \begin{itemize}
          ...
          \item Rich library of optimised ansatz forms
        \end{itemize}
      \end{block}
  \end{columns}
  ...
  \begin{alertblock}{Outlook}
    ...
    Cross-fertilisation between the two frameworks is the key direction.
  \end{alertblock}
\end{frame}
