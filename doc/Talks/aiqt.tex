...
\date{Japan-Norway Research Alliance – Next Generation Research with AI, September 18, 2026}
...
{\small
Niyaz Beysengulov (EeroQ), Stian Bilek, Antoine Camper, Jonas Flaten, Gunnar Lange, Oskar Leinonen, Jan Malamant, Francesco Massel, Heidi Sandaker, Viktor Svensson (UiO), Johannes Pollanen (MSU/EeroQ), Zachary Stewart and Jared Weidman (MSU), Angela Wilson (MSU/Texas AM), Maria Elena Perruzza (UiA/USN),  and the CERN DRD5 collaboration on quantum sensors.
Jane Kim (ANL), Patrick Cook, Danny Jammooa, Dean Lee, Bryce Fore (ANL), Alessandro Lovato (ANL), and Giuseppe Carleo (EPFL). Excuses to those I have omitted.
}
\vspace{2mm}
\begin{block}{Sponsors}
University of Oslo, NSF (US) and DOE (US) and Michigan State University.
\end{block}
\end{frame}
...
\centerline{\includegraphics[width=1.0\linewidth]{figureintro.pdf}}
\end{frame}
...
\begin{frame}[plain,fragile]
\frametitle{Agents for AI: from predicting to acting}
\small
\begin{block}{}
An \textbf{AI agent} does not just make a prediction: it \emph{observes} its environment, \emph{acts}, receives \emph{feedback}, and repeats,
$s_t \rightarrow a_t\sim\pi_{\bm{\theta}}(\cdot\,|\,s_t) \rightarrow r_t \rightarrow \text{update } \bm{\theta}$.
\end{block}
\begin{columns}[t]
\column{0.5\linewidth}
\begin{block}{Two flavours}
\begin{itemize}
\item \textbf{RL agents:} a neural-network policy $\pi_{\bm{\theta}}$ trained to maximise a reward; actions are numbers (voltages, pulse amplitudes)
\item \textbf{Language-model agents:} an LLM that plans, writes and runs code, and uses simulators and instruments as \emph{tools}
\end{itemize}
\end{block}
\column{0.5\linewidth}
\begin{block}{In quantum control today}
\begin{itemize}
\item Control pulses and gates beyond human-crafted protocols
\item Autonomous qubit tune-up and calibration (quantum dots, superconducting circuits)
\item Real-time quantum error correction run by an RL agent
\item Closed-loop experiments: agent proposes, hardware answers
\end{itemize}
\end{block}
\end{columns}
\vspace{1mm}
{\scriptsize Our example later: the environment is a physics simulator, the actions are electrode voltages, and the agent designs a quantum sensor.\\ Bukov et al., PRX \textbf{8}, 031086 (2018); Niu et al., npj Quantum Inf.\ \textbf{5}, 33 (2019); Sivak et al., Nature \textbf{616}, 50 (2023); Zwolak and Taylor, Rev.\ Mod.\ Phys.\ \textbf{95}, 011006 (2023).}
\end{frame}
...
Both are optimisation problems in exponentially large spaces; both need scientists who speak both languages.
\end{block}
\end{frame}

\begin{frame}[plain,fragile]
\frametitle{AI as a tool for quantum engineering}
\centerline{\includegraphics[width=1.0\linewidth]{krenn1.png}}
...
\centerline{\includegraphics[width=1.0\linewidth]{mbpfig7.png}}
...
\centerline{\includegraphics[width=0.95\linewidth]{fig3.pdf}}
...
\column{0.5\linewidth}
\centerline{\includegraphics[width=1.0\linewidth]{fig6.pdf}}
...
\centerline{\includegraphics[width=0.9\linewidth]{fig7.pdf}}
...
\centerline{\includegraphics[width=0.9\linewidth]{fig4.pdf}}
